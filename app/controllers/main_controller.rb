...
class MainController < ApplicationController
  ...
  def login
    if correct_pincode? and admin?
      session[:home_url] = root_path if !session[:home_url]
  ...
  # 为图表预备需要的资料
  def prepare_chart_data( chart_period, chart_data_size, symbol = "btcusdt" )
    if @raw_data = get_kline(chart_period,chart_data_size,symbol) and @raw_data.size > 0
  ...
  # 显示K线图
  def kline_chart
    symbol = params[:symbol] ? "btcusdt" : params[:symbol]
    if prepare_chart_data($default_chart_period,$chart_data_size,symbol)
  ...
  def read_auto_invest_log
    read_log_func($auto_invest_log_path)
  end

  # 同上，添加读取ETH交易讯息
  def read_auto_invest_eth_log
    read_log_func($auto_invest_eth_log_path)
  ...
  def set_auto_sell_eth_form
    ...
    if get_invest_params(1,'ETH') == 'usdt'
      ...
      @price_pos = 0              # 价格小数点位数
      @price_step = 5             # 价格递增值
    elsif get_invest_params(1,'ETH') == 'btc'
      @price_now = get_ethbtc_price
      @price_pos = 5
      @price_step = 0.00005
    end
  end
  ...
  def execute_setup_invest_param( code = params[:c], index = params[:i], value = params[:v] )
    code = "BTC" if code == nil
    if index and index.to_i < $exe_auto_invest_params_size and value
      set_invest_params(code,index,value)
      ...
      # 定价卖出的策略和卖最高价的策略无法并存
      if index.to_i == 18 and get_invest_params(18,code).to_i > 0
        set_invest_params(code,27,0)
      elsif index.to_i == 27 and get_invest_params(27,code).to_i > 0
        set_invest_params(code,18,0)
      end
      put_notice show_set_auto_invest_params_ok index, value
      # 交易列表记录日期必须在三日以内
      today = Date.today
      if (today-get_invest_params(9,code).to_date).to_i > 3
        set_invest_params(code,9,(today-2.day).to_s(:db))
      end
    else
  ...
  def show_set_auto_invest_params_ok( index = nil, value = nil )
    # info = (index and value) ? "(#{index} = #{value}) 现价每笔投资¥#{single_invest_cost.to_i} 最低价每笔投资¥#{max_invest_cny.to_i}" : ""
  ...
  def buy_fall_list
    ...
    @eq_btc_amount = get_btc_eth_amounts # 该账号BTC数量
