class TradeStrategiesController < ApplicationController
    ...
    def index
      @trade_strategies = TradeStrategy.all.order(:param_name)
    end
    ...
    # 同步另一台服务器的值
    def sync_attributes
      send_sync_request "#{sync_root}&deal_type=#{params[:trade_strategy][:deal_type]}&param_name=#{params[:trade_strategy][:param_name]}&param_value=#{params[:trade_strategy][:param_value]}&range_step=#{params[:trade_strategy][:range_step]}"
    end
  ...
  private
    ...
    def trade_strategy_params
      params.require(:trade_strategy).permit(:deal_type, :param_name, :param_value, :range_step)
    end

    def sync_root
      "#{$host2}sync_tstrategy_info.json?key=#{$api_key}&sync_code=#{[@trade_strategy.deal_type,@trade_strategy.param_name].join(',')}"
    end
