...
if TEST_ENV:
    ...
    CONN = sqlite3.connect(db_path())
# 设定文档路径
PARAMS = get_doge_params_path()
# Log文档路径
LOG_FILE = 'auto_invest_doge_log.txt'
...
def get_price_now():
    global target_bi
    try:
        return float(get_kline('dogebtc', '1min', 1)['data'][0]['close'])
    except:
        return 0
...
def place_new_order(price, amount, deal_type):
    ...
    try:
        if TEST_ENV:
            ...
            root = send_order(amount, "api", 'dogebtc', deal_type, price)
            if root["status"] == "ok":
                ORDER_ID = root["data"]
...
def clear_orders():
    global target_bi
    try:
        root = cancel_open_orders(ACCOUNT_ID, 'dogebtc')
        if root["status"] == "ok":
    ...
    if test_price == 0:
        ...
        if deal_type.find('sell-limit') > -1:
            ...
            if trade_usdt > 0:
                ...
                # 显示目前BTC的剩余数量与持仓水平
                msg_str = "BTC Now: %.8f Level: %.2f%%" % (trade_btc, btc_hold_level())
                print(msg_str)
                ftext += msg_str+'\n'
                # 更新火币交易所的资产现况并显示
...
def btc_hold_level():
    ...
    return amounts['btc']/ori_btc*100
...
# 执行卖出下单的主要流程
def buy_process(test_price, price_now, ftext, time_line, u2c, buy_eq_cny):
    global ORDER_ID         # 下单回传编号
    global FORCE_BUY       # 是否执行强制卖出
    global acc_id           # 交易所识别账号
    global stop_buy_level  # 停止自动卖出的仓位值
    # 卖出后仓位必须大于保留的仓位值
    if safe_after_buy(price_now):
        # 下单的买入价比市价稍高以争取快速成交
        buy_price = round(price_now*buy_price_rate(), 10)
        # 买入的总量
        buy_amount = cal_buy_amount(price_now)
        ftext = place_order_process(test_price, buy_price, buy_amount, \
        'buy-limit', ftext, time_line, u2c)
        # 如果只是测试，则测试完后退出
        if TEST_ENV:
            return ftext
        ...
        if test_price == 0 and len(ORDER_ID) > 0:
            ...
            while True:
                # 如果回传值大于0表示已经成交
                n_dl_added = update_huobi_doge_deal_records(time_line)
                ...
                if WAIT_SEND_SEC*count > MAX_WAIT_SELL_SEC and len(ORDER_ID) > 0:
                  cancel_order(ORDER_ID)
                  msg_str = "Send Buy Order(%s) canceled!" % ORDER_ID
                  print(msg_str)
                  ftext += msg_str+'\n'
                  break
            # 重置交易单号
...
def get_min_max_price(buy_price_period, sell_price_period):
    ...
    try:
        ...
        if buy_price_period == 0:
            buy_price_period = 1
        root = get_huobi_price('dogebtc', '1min', buy_price_period)
        ...
        if sell_price_period == 0:
            sell_price_period = 1
        root = get_huobi_price('dogebtc', '1min', sell_price_period)
        for data in root["data"]:
...
# 求解买入几分钟内的最低价，返回现价、最低价、是否已达到触发条件
def min_price_in(size):
    try:
        a = []
        if size == 0:
            size = 1
        root = get_huobi_price('dogebtc', '1min', size)
...
def last_buy_interval():
    global acc_id
    try:
        sql = ("SELECT updated_at FROM deal_records WHERE account = '%s' and deal_type = 'buy-limit' and symbol = 'dogebtc' ORDER BY updated_at DESC LIMIT 1" % acc_id)
        rows = select_db(sql)
...
# 预计要买入的BTC总量
def cal_buy_amount(price_now):
    global buy_eq_cny
    # 买入后得DOGE
    buy_eq_cny/usdt2cny()/btc_price_now()/price_now


# 试算买入后是否还维持在停止自动买入的仓位值之上
def safe_after_buy(price_now):
    global ori_btc
    global stop_buy_level
    # 下一次预计要买入的BTC总量
    buy_amount = cal_buy_amount(price_now)
    # 判断买入后的BTC总量 > 原有BTC总量*停止自动买入的仓位值 即可！
    ori_amount = get_btc_amount_ex() # 交易所目前剩余的BTC总量
    after_amount = ori_amount - buy_amount # 买入后的BTC总量
    if after_amount > ori_btc*stop_buy_level/100.0:
        return True
    else:
        return False
...
# 执行自动下单的主程序
def exe_auto_invest(bottom_price, factor, max_buy_level, min_usdt, max_usdt, time_line, test_price, profit_goal, min_sec_rate, max_sec_rate):
    # 引用全域变数
    global ORDER_ID
    global FORCE_BUY
    global FORCE_SELL
    ...
    global min_price
    global buy_price_period
    global every_sec_for_buy
    global acc_id
    global stop_buy_level
    global reduce_step
    global top_price
    global force_buy_price
    global buy_eq_cny
    # 写入Log文档
    with open(LOG_FILE, 'a') as fobj:
        ...
        u2c = usdt2cny()
        if force_buy_price > 0:
            msg_str = "%s:%.6f Invest Every %i Sec When Price Below %.6f" % (get_now(), price_now, every_sec_for_buy, force_buy_price)
        else:
            msg_str = "%s:%.6f Invest Every %i Sec When Price Below %i Minutes Min Price" % (get_now(), price_now, every_sec_for_buy, buy_price_period)
        if price_now > 0:
            ftext += msg_str+'\n'
            # 必须大于等于最小买入值才会送单
            if buy_eq_cny >= MIN_TRADE_USDT*u2c and FORCE_BUY == True:
                ftext = buy_process(0, price_now, ftext, time_line, \
                        u2c, buy_eq_cny)
                fobj.write(ftext)
                return every_sec_for_buy
            else:
                fobj.write(ftext)
                return every_sec_for_buy
        else:
    ...
    while True:
        ...
        try:
            ...
            with open(PARAMS, 'r') as fread:
                ...
                every_sec, target_bi, bottom_price, ori_btc, factor, max_buy_level, target_amount, min_usdt, max_usdt, deal_date, deal_time, test_price, profit_goal, max_sell_count, min_sec_rate, max_sec_rate, detect_sec, buy_price_period, sell_price_period, buy_period_move, force_to_sell, min_price_index, every_sec_for_buy, sell_max_cny, acc_id, deal_cost, deal_amount, force_buy_price, acc_real_profit, stop_sell_level, force_to_buy, buy_period_max, is_lower_ave, reduce_step, top_price, trace_min_rate, trace_max_rate, top_price_minutes, sell_period_min, buy_eq_cny = params_str.split(' ')
                # 将设定文档参数根据适当的型别初始化
                every_sec = int(every_sec)
                bottom_price = int(bottom_price)
                ori_btc = float(ori_btc)
                factor = float(factor)
                ...
                every_sec_for_buy = int(every_sec_for_buy)
                sell_max_cny = int(sell_max_cny)
                force_buy_price = float(force_buy_price)
                ...
                # 每笔卖出值多少人民币
                buy_eq_cny = float(buy_eq_cny)
                # 获得在几分钟内狗狗币价格的最大值与最小值
                    ...
                    for remaining in range(int(code), 0, -1):
                        ...
                        if remaining % detect_sec == 0:
                            ...
                            with open(LOG_FILE, 'a') as fa:
                                ###############################################################
                                # 现价、最高价、是否已达到买价
                                price_now, min_price, over_min_price = min_price_in(buy_price_period)
                                # 是否达到可买入的时间
                                over_sell_time = last_buy_interval() > every_sec_for_buy
                                # 是否在可买入的价格之上
                                is_below_price = price_now > 0 and force_buy_price > 0 and price_now <= force_buy_price
                                # 是否达到分钟内的最高价
                                reach_low_price = buy_price_period > 0 and over_max_price
                                ...
                                #################################################################
                                # 达到卖出的条件则执行卖出
                                # 买入几分钟内的最低价时是否不高于所设定的最高价
                                if buy_period_max > 0:
                                    if price_now <= buy_period_max:
                                        below_max_buy_price = True
                                    else:
                                        below_max_buy_price = False
                                else:
                                    below_max_buy_price = False
                                # 如果破底后，检查是否涨到想买入的价位
                                if below_max_buy_price and \
                                    (reach_low_price or TRACE_MAX_PRICE > 0):
                                ...
                                if price_now > 0 and max_price > 0 and sell_price_period > 0:
                                    if TEST_ENV:
                                        ...
                                        env = ''
                                    msg_str = "%s%s:%.6f %imax:%.6f t&b_%.6f:%s over_time:%s over_min_price:%s level_after_buy:%s" % (env, get_now(), price_now, buy_price_period, min_price,  TRACE_MIN_PRICE*TRACE_MIN_RATE, is_sell_price, over_sell_time, below_max_buy_price, is_safe_after_sell)
                                    stdout_write(msg_str)
                                if is_safe_after_buy and over_buy_time and TRACE_MIN_PRICE > 0:
                                    fa.write(msg_str+'\n')
                                # 买入条件：仓位、时间、获利、[可买价格之下|买入分钟内的最低价且到达买入价]
                                if is_safe_after_buy and over_buy_time and (is_below_price or is_buy_price):
                                    FORCE_BUY = True
                                    break
                    stdout_write(" "*30)
        except:
