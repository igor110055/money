...
def btc_ave_cost():
    global acc_id
    rows = select_db("SELECT price, amount, fees FROM deal_records WHERE account = '%s' and auto_sell = 0 and deal_type = 'buy-limit' and symbol = 'btcusdt'" % acc_id)
    if len(rows) > 0:
...
def batch_sell_process(test_price, price, base_price, ftext, time_line, u2c, profit_goal, max_sell_count):
    ...
    if max_sell_count > 0 and safe_after_sell(price, stop_sell_level):
        # 读取未卖交易记录
        rows = select_db(
            "SELECT id, amount - fees as amount, created_at, price FROM deal_records WHERE account = '%s' and auto_sell = 0 and deal_type = 'buy-limit' and symbol = 'btcusdt' ORDER BY first_sell DESC, price ASC LIMIT %i" % (acc_id, max_sell_count))
        len_rows = len(rows)
        # 如果有未卖的交易记录
        if len_rows > 0:
            ids = []                # 存放数据ID
...
def last_buy_interval():
    ...
    try:
        rows = select_db(
            "SELECT created_at FROM deal_records WHERE account = '%s' and auto_sell = 0 and deal_type = 'buy-limit' and symbol = 'btcusdt' ORDER BY created_at DESC LIMIT 1" % acc_id)
...
def last_sell_interval():
    ...
    try:
        rows = select_db(
            "SELECT updated_at FROM deal_records WHERE account = '%s' and auto_sell = 1 and deal_type = 'buy-limit' and symbol = 'btcusdt' ORDER BY updated_at DESC LIMIT 1" % acc_id)
        start_time = datetime.strptime(rows[0][0], "%Y-%m-%d %H:%M:%S")
        end_time = datetime.strptime(get_now(), "%Y-%m-%d %H:%M:%S")
        total_seconds = (end_time - start_time).total_seconds()
        return int(total_seconds)
...
def top_n_profit(sell_price):
    ...
    rows = select_db("SELECT amount - fees as amount, price FROM deal_records WHERE account = '%s' and auto_sell = 0 and deal_type = 'buy-limit' and symbol = 'btcusdt' ORDER BY first_sell DESC, price ASC LIMIT %i" % (acc_id, max_sell_count))
    if len(rows) > 0:
        sell_profit_total = 0
        fr = fees_rate()
        u2c = usdt2cny()
...
def top_n_amount():
    global acc_id
    global max_sell_count
    rows = select_db("SELECT amount - fees FROM deal_records WHERE account = '%s' and auto_sell = 0 and deal_type = 'buy-limit' and symbol = 'btcusdt' ORDER BY first_sell DESC, price ASC LIMIT %i" % (acc_id, max_sell_count))
    if len(rows) > 0:
        total_amount = 0
        for row in rows:
            total_amount += row[0]
                            ...
                            with open(LOG_FILE, 'a') as fa:
                                ...
                                if safe_after_sell(price_now, stop_sell_level):
                                    ...
                                    if safe_after_sell_msg_flag:
                                        safe_after_sell_msg_flag = False
                                        msg_str = "%s:%.2f ^_^ 卖出后仓位在预想范围内，程序可以执行自动卖出了" % (get_now(), price_now, stop_sell_level)
                                        fa.write(LINE_MARKS+'\n'+msg_str+'\n')
                                elif not safe_after_sell_msg_flag:
                                    is_safe_after_sell = False
                                    msg_str = "%s:%.2f >_< 由于卖出后仓位小于保留的仓位值(%i%%)，请确认现货余额大于订单总额，或调低保留的仓位值，或调低卖出总量以便程序可以执行自动卖出！" % (get_now(), price_now, stop_sell_level)
                                    fa.write(LINE_MARKS+'\n'+msg_str+'\n')
