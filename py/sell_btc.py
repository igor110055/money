...
def get_price_now():
    global target_bi
    try:
        return float(get_kline('btc'+target_bi, '1min', 1)['data'][0]['close'])
    except:
        return 0
...
def place_new_order(price, amount, deal_type):
    ...
    try:
        if TEST_ENV:
            ...
            root = send_order(amount, "api", 'btc'+target_bi, deal_type, price)
            if root["status"] == "ok":
                ORDER_ID = root["data"]
...
def clear_orders():
    global target_bi
    try:
        root = cancel_open_orders(ACCOUNT_ID, 'btc'+target_bi)
        if root["status"] == "ok":
...
def sell_process(test_price, price_now, ftext, time_line, u2c, sell_eq_cny):
    ...
    if safe_after_sell(price_now):
        ...
        if test_price == 0 and len(ORDER_ID) > 0:
            ...
            while True:
                # 如果回传值大于0表示已经成交
                n_dl_added = update_huobi_btc_deal_records(time_line)
...
def get_min_max_price(buy_price_period, sell_price_period):
    ...
    try:
        ...
        if buy_price_period == 0:
            buy_price_period = 1
        root = get_huobi_price('btc'+target_bi, '1min', buy_price_period)
        ...
        if sell_price_period == 0:
            sell_price_period = 1
        root = get_huobi_price('btc'+target_bi, '1min', sell_price_period)
        for data in root["data"]:
...
def min_price_in(idx, size):
    ...
    try:
        ...
        if size == 0:
            size = 1
        root = get_huobi_price('btc'+target_bi, '1min', size)
...
def last_sell_interval():
    ...
    global target_bi
    try:
        sql = ("SELECT updated_at FROM deal_records WHERE account = '%s' and deal_type = 'sell-limit' and symbol = 'btc%s' ORDER BY updated_at DESC LIMIT 1" % acc_id, target_bi)
        rows = select_db(sql)
...
def safe_after_sell(price_now):
    ...
    # 下一次预计要卖出的BTC总量
    sell_amount = cal_sell_amount(price_now)
    # 判断卖出后的BTC总量 > 原有BTC总量*停止自动卖出的仓位值 即可！
    ori_amount = get_btc_amount_ex() # 交易所目前剩余的BTC总量
    after_amount = ori_amount - sell_amount # 卖出后的BTC总量
    if after_amount > ori_btc*stop_sell_level/100.0:
        return True
    else:
        return False
    ...
    while True:
        try:
            with open(PARAMS, 'r') as fread:
                ...
                every_sec, target_bi, bottom_price, nouse1, factor, max_buy_level, target_amount, min_usdt, max_usdt, deal_date, deal_time, test_price, profit_goal, max_sell_count, min_sec_rate, max_sec_rate, detect_sec, buy_price_period, sell_price_period, buy_period_move, force_to_sell, min_price_index, every_sec_for_sell, ori_btc, acc_id, deal_cost, deal_amount, force_sell_price, acc_real_profit, stop_sell_level, force_to_buy, buy_period_max, is_lower_ave, reduce_step, top_price, trace_min_rate, trace_max_rate, top_price_minutes, sell_period_min, sell_eq_cny = params_str.split(' ')
                # 将设定文档参数根据适当的型别初始化
                target_bi = "usdt"
